...
def inference(wav_path, embedding_per_second=1.0, overlap_rate=0.5):
    ...
    network_eval.load_weights(
        r"speakerDiarization/diarization/ghostvlad/pretrained/weights.h5", by_name=True
    )
    ...
    time_spec_rate = (
        1000 * (1.0 / embedding_per_second) * (1.0 - overlap_rate)
...
def simple_upload(request):
    if request.method == "POST" and request.FILES["file"]:
        ...
        fs = FileSystemStorage(
            location="speakerDiarization/wavs", base_url="speakerDiarization/wavs"
        )
        ...
        data = inference(
            wav_path=r"speakerDiarization/wavs/" + filename,
