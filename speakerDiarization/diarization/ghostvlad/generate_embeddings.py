...
parser.add_argument('--data_path', default='VoxCeleb/', type=str)
...
def prepare_data(SRC_PATH):
    ...
    for i,spkDir in enumerate(wavDir):   # Each speaker's directory
        ...
        wavPath_list = os.listdir(os.path.join(SRC_PATH, spkDir))
        
        for wavPath in wavPath_list:
            wavPath = os.path.join(SRC_PATH, spkDir, wavPath)
...
def main():
    ...
    SRC_PATH = r'VoxCeleb/vox1_test_wav/wav'
    ...
    with open('train_sequence.pkl','wb') as f:
        pickle.dump(train_sequence, f)
    with open('train_cluster_id.pkl','wb') as f:
        pickle.dump(train_cluster_id, f)
